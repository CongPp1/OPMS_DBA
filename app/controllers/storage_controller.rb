...
class StorageController < ApplicationController
  ...
  def storage_usage_segments
    @segments = sql_select_all "SELECT /* OPMS-Tool */
                                       Segment_Type,
  ...
  def storage_usage_tablespaces_per_schema
    @tablespace_per_schema = sql_select_all "
      ...
      SELECT /* OPMS-Tool */ s.Owner, s.Tablespace_Name, s.MBytes, q.Bytes Bytes_Charged, q.Max_Bytes Bytes_Quota
      FROM (
  ...
  def storage_usage_schemas
    schemas = sql_select_all("\
      SELECT /* OPMS-Tool */ Owner Schema, Type Segment_Type, SUM(Bytes)/1048576 MBytes
      FROM (
        SELECT s.Owner,
  ...
  def undo_usage
    ...
    @undo_segments = sql_select_iterator("\
      SELECT /* OPMS-Tool */ r.*, i.Owner Extent_Owner, i.Size_MB, i.Size_MB_UnExpired, i.Size_MB_Expired, i.Size_MB_Active, t.Transactions
  ...
  def temp_usage
    ...
    @temp_ts_size = sql_select_one "\
        ...
        FROM   (
                SELECT /* OPMS-Tool */  SUM(d.Bytes)/1048576 Size_MB
  ...
  def list_exadata_cell_grid_disk
    ...
    @disks = sql_select_all ["\
      SELECT /* OPMS-Tool */ pd.*, cd.*, gd.*, adg.Name ASM_Disk_Group_Name
      FROM (
                        SELECT /*+ NO_MERGE */
                                c.cellname
                              , CAST(EXTRACTVALUE(VALUE(v), '/celldisk/name/text()')                              AS VARCHAR2(100)) cd_name
