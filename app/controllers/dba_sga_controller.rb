...
class DbaSgaController < ApplicationController
  ...
  def list_bind_variables
    ...
    @binds = sql_select_all ["\
      SELECT /* OPMS-Tool */ Child_Number, Name, Position, DataType_String, Last_Captured,
  ...
  def list_sql_shared_cursor
    ...
    @reasons = sql_select_all ["\
      SELECT /* OPMS-Tool */ * FROM gv$SQL_Shared_Cursor WHERE Inst_ID=? AND SQL_ID=?",
  ...
  def list_compare_execution_plans
    ...
    @plan_count = sql_select_one ["\
            SELECT /* OPMS-Tool */ COUNT(DISTINCT Plan_Hash_Value)
            FROM  gV$SQL_Plan p
            WHERE (    Inst_ID      = ?
                   AND SQL_ID       = ?
                   AND Child_Number = ?
