  ...
  def object_nach_wait_parameter(detail_update_area, instance, event, p1, p1raw, p1text, p2, p2raw, p2text, p3, p3raw, p3text)
    ...
    when p1text=="address" && p2text=="number" && p3text=='tries' then
      ...
      result = sql_select_one ["\
        SELECT /*+ ORDERED USE_NL(b o) */ /* OPMS-Tool */
    ...
    when p1text == "idn" && p2text == "value" && p3text == "where"
      if event.match('cursor: ')
        cursor_rec = sql_select_first_row ["SELECT /*+ OPMS-Tool Ramm */ SQL_ID, Parsing_Schema_Name, SQL_Text FROM gv$SQL WHERE Inst_ID=? AND Hash_Value=?", instance, p1.to_i]
        ...
        lc_obj_name = sql_select_one ["SELECT /*+ OPMS-Tool Ramm */ Type||' '||Owner||'.'||Name FROM gv$DB_Object_Cache WHERE Inst_ID=? AND Hash_Value=?", instance, p1.to_i]
